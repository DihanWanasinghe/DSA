...
//        arrangeHeap(lastParentIndex);
    ...
    private void heapifyUp(int currentIndex){
        ...
        if(temp.getDistance() <heapArray.get(parentIndex).getDistance()){
    ...
    private void heapifyDown(int currentIndex){
        ...
        if( (2*currentIndex+2) <lastIndex && heapArray.get(lowestChildIndex).getDistance() > heapArray.get(lowestChildIndex+1).getDistance() ) {

            lowestChildIndex = 2 * currentIndex + 2 ;
        }


        if(heapArray.get(lowestChildIndex).getDistance() < heapArray.get(currentIndex).getDistance()){
    ...
    public void decreaseKey(Vertex v, int newDistance) {
        ...
        heapArray.get(index).setDistance(newDistance);
        heapifyUp(index);
    }
